...
typedef struct cost_map {
  character_type_t character;
  int costs[21][80];
} cost_map_t;

static int move_costs[num_character_types][num_terrain_types] = {
...
static int getCost(terrain_type_t ter, character_type_t char_type){
  return move_costs[char_type][ter];
}

static void dijkstra_map_cost(map_t *m, character_type_t char_type, int pc_x, int pc_y, int costs[][MAP_X])
{
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      path[y][x].cost = 999999999;
    }
  }

  path[pc_x][pc_y].cost = 0;

  heap_init(&h, path_cmp, NULL);

  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      if(m->map[y][x] != ter_boulder){
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         (p->cost + getCost(m->map[p->pos[dim_y] - 1]
                                  [p->pos[dim_x]    ], char_type)))) 
    { 
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        ((p->cost + getCost(m->map[p->pos[dim_y] - 1]
                                  [p->pos[dim_x]    ], char_type)));
    ...
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost >
         ((p->cost + getCost(m->map[p->pos[dim_y]][p->pos[dim_x] - 1], char_type))))) {
      path[p->pos[dim_y]][p->pos[dim_x] - 1].cost =
        ((p->cost + getCost(m->map[p->pos[dim_y]]
                                  [p->pos[dim_x] - 1], char_type)));
    ...
    if ((path[p->pos[dim_y] + 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] + 1][p->pos[dim_x]    ].cost >
         ((p->cost + getCost(m->map[p->pos[dim_y] + 1][p->pos[dim_x]], char_type))))) {
      path[p->pos[dim_y] + 1][p->pos[dim_x]    ].cost =
        ((p->cost + getCost(m->map[p->pos[dim_y] + 1][p->pos[dim_x]], char_type)));
      heap_decrease_key_no_replace(&h, path[p->pos[dim_y] + 1]
                                           [p->pos[dim_x]    ].hn);
    ...
    if ((path[p->pos[dim_y] + 1][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y] + 1][p->pos[dim_x] - 1].cost >
         ((p->cost + getCost(m->map[p->pos[dim_y] + 1][p->pos[dim_x] - 1], char_type))))) {
      path[p->pos[dim_y] + 1][p->pos[dim_x] - 1].cost =
        ((p->cost + getCost(m->map[p->pos[dim_y] + 1][p->pos[dim_x] - 1], char_type)));
      heap_decrease_key_no_replace(&h, path[p->pos[dim_y] + 1]
                                           [p->pos[dim_x] - 1].hn);
...
static void print_map(map_t *m, int pc_x, int pc_y)
{
  int x, y;
...
void print_distance_map(int costs[][MAP_X]){
  int i, j;
  ...
  for(i = 0; i < MAP_Y; i++){
    for(j = 0; j < MAP_X; j++){
      if(costs[i][j] == INT_MAX){
        ...
        printf("%d ", costs[i][j] % 100);
      }
    }
  }
}

void place_player(map_t* m, int *pc_x, int *pc_y){
  int x, y;
  while(1){
    x = rand() % MAP_Y;
    y = rand() % MAP_X;
    if(m->map[x][y] == ter_path){
      break;
    }
  }
  *pc_x = x;
...
int main(int argc, char *argv[])
{
  ...
  int hiker_map[21][80];
  int rival_map[21][80];
  character_type_t hiker = char_hiker;
  character_type_t rival = char_rival;
  int pc_x, pc_y;
  ...
  new_map(&d);
  place_player(&d, &pc_x, &pc_y);
  print_map(&d, pc_x, pc_y);
  dijkstra_map_cost(&d, hiker, pc_x, pc_y, hiker_map);
  dijkstra_map_cost(&d, rival, pc_x, pc_y, rival_map);
  print_distance_map(hiker_map);
  print_distance_map(rival_map);
